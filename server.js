...
app.post("/api/support", async (req, res) => { try { const message = String(req.body.message || "").trim(); const contact = String(req.body.contact || "").trim(); if (!message) return res.status(400).json({ error: "Введите сообщение" }); if (message.length > 4000) return res.status(400).json({ error: "Слишком длинное сообщение" }); const botToken = process.env.TELEGRAM_BOT_TOKEN; const chatId = process.env.SUPPORT_CHAT_ID; if (!botToken || !chatId) return res.status(500).json({ error: "Поддержка не настроена" }); const nl = String.fromCharCode(10); const text = "🧶 Поддержка (сайт Схемы)" + nl + nl + message + (contact ? (nl + nl + "Контакт: " + contact) : "") + nl + nl + "Страница: " + (req.headers.referer || req.headers.origin || "—"); const tg = await fetch("https://api.telegram.org/bot" + botToken + "/sendMessage", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: chatId, text: text }) }); const tj = await tg.json().catch(() => ({})); if (!tg.ok || !tj.ok) { console.error("TG error:", JSON.stringify(tj)); return res.status(502).json({ error: "Не удалось отправить" }); } res.json({ ok: true }); } catch (e) { console.error(e); res.status(500).json({ error: "Ошибка сервера" }); } });  const PORT = process.env.PORT || 3000;
